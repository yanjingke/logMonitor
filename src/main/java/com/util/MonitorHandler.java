...
import com.bean.*;
import com.dao.LogMonitorDao;
import com.mail.MailInfo;
import com.mail.MessageSender;
...
public class MonitorHandler {
    ...
    private static Map<String, List<User>> loadUserMap(){
        ...
        for (App app:applist){
            ...
            List<User> usersInapp = map.get(app.getId());
            if(usersInapp==null){
                usersInapp=new ArrayList<User>();
                map.put(app.getId()+"",usersInapp);
            }
            String[]userIDArr=userIds.split(",");
            for (String userId:userIDArr){
                usersInapp.add(queryUserById(userId));
            }
            map.put(app.getId() + "", usersInapp);
        }
        return map;
